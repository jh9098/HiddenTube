...
# ===================== (여기에 본인 YouTube Data API 키 직접 입력) =====================
#API_KEY = "AIzaSyCwPdZQJDXBxcXMvM5gzMZJiuh8Y8uY3X8"  # ← 반드시 본인 API 키로 교체하세요.
#AIzaSyDvBR7AIX-lCzLKbkjhe8TqccO8-JT6dBQ

#AIzaSyB7MrXpKoE8lkNTo2XJmzTOmQNaDXpovec
# (선택) 폴백용 채널 화이트리스트(쿼터 초과 시 사용). 채널 ID(UC...)를 추가해두면 정확도 ↑
# -*- coding: utf-8 -*-

import os, re, json, threading, tkinter as tk, datetime as dt
...
# ===================== 설정 =====================
API_KEY = "AIzaSyDvBR7AIX-lCzLKbkjhe8TqccO8-JT6dBQ"  # ← 직접 넣으세요 (환경변수보다 우선)
FALLBACK_CHANNELS = []  # (선택) API 쿼터 초과 시 사용할 채널ID 목록(UC… 형식)

DATA_DIR = "data"
CHANNEL_STORE_PATH = os.path.join(DATA_DIR, "channels.json")  # 채널ID 저장소
...
def _build_ydl_opts(base_opts: dict = None) -> dict:
    ...
    if base_opts: ydl_opts.update(base_opts)
...
def clean_vtt(vtt_text: str) -> str:
    lines = vtt_text.splitlines(); result = []; prev = ""
    for line in lines:
        if _CUE_RE.match(line): continue
        if line.strip().startswith(("WEBVTT","Kind:","Language:","NOTE","align:","position:")): continue
        clean = re.sub(r"<[^>]+>", "", line).strip()
        if clean and clean != prev:
            result.append(clean); prev = clean
    return "\n".join(result)


def extract_text_and_title(youtube_url: str, cookie_path=''):
    opts = {'skip_download': True,'writesubtitles': True,'writeautomaticsub': True,
            'quiet': True,'forcejson': True,'simulate': True,
            'http_headers': {'User-Agent': 'Mozilla/5.0'}}
    if cookie_path and os.path.exists(cookie_path): opts['cookiefile'] = cookie_path
    ydl_opts = _build_ydl_opts(opts)
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        info = ydl.extract_info(youtube_url, download=False)
        title = info.get('title') or "video"
        subs = info.get('subtitles') or {}; auto_subs = info.get('automatic_captions') or {}
        def get_vtt(sub_dict, preferred=('ko','ko-KR','ko_KR','en')):
            for lang in preferred:
                if lang in sub_dict:
                    for f in sub_dict[lang]:
                        if f.get('ext') == 'vtt': return ydl.urlopen(f['url']).read().decode('utf-8')
            for _, formats in sub_dict.items():
                for f in formats:
                    if f.get('ext') == 'vtt': return ydl.urlopen(f['url']).read().decode('utf-8')
            return None
        vtt_text = get_vtt(subs) or get_vtt(auto_subs)
        if not vtt_text: return None, title
...
def _parse_iso8601_duration_to_seconds(iso_dur: str) -> int:
    if not iso_dur or not iso_dur.startswith("PT"): return 0
    h=m=s=0
...
def _to_iso_utc_datetime(date_str, hour, minute):
...
def save_channel_store(data):
    _ensure_data_dir()
    with open(CHANNEL_STORE_PATH, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)


def add_channel_to_store(cid: str, title: str):
    """UC... ID만 저장. 타이틀은 최신 값으로 갱신."""
    if not cid or not cid.startswith("UC"): return
...
def remove_channels_from_store(ids):
    data = load_channel_store()
...
def _resolve_channel_id(youtube, channel_input: str) -> str:
    """입력을 가능하면 채널ID(UC…)로 해석. (@핸들/URL 허용)"""
    if not channel_input: return ""
    s = channel_input.strip()
    if _CHANNEL_ID_RE.match(s): return s
    if 'youtube.com' in s:
...
def search_youtube_videos_api(keyword, max_results=50, time_filter="any",
                              custom_from="", custom_to="", duration_filter="any",
                              sort_by="views", channel_filter=""):
    """
    반환: [ [url, title, date_raw, date_fmt, channel_title, view_count, dur_seconds, channel_id], ... ]
    ※ UI에는 채널명만 표기, channel_id는 메타로 보관(자동 저장 안 함).
    """
    max_results = max(1, min(int(max_results), 50))
    api_key = _resolve_api_key()
    if not api_key: raise RuntimeError("YouTube API 키가 설정되지 않았습니다.")
    if build is None: raise RuntimeError("google-api-python-client가 필요합니다.")
    ...
    if time_filter in ("day","week","month","custom"):
        if time_filter=="day": params['publishedAfter'] = (now_utc - dt.timedelta(days=1)).isoformat()
        elif time_filter=="week": params['publishedAfter'] = (now_utc - dt.timedelta(weeks=1)).isoformat()
        elif time_filter=="month": params['publishedAfter'] = (now_utc - dt.timedelta(days=30)).isoformat()
        elif time_filter=="custom":
            if custom_from: params['publishedAfter'] = custom_from
            if custom_to: params['publishedBefore'] = custom_to
    ...
    for v in videos_resp.get('items', []):
        vid = v.get('id'); sn = v.get('snippet',{}) or {}; st = v.get('statistics',{}) or {}; cd = v.get('contentDetails',{}) or {}
        ...
        try: vc = int(st.get('viewCount')) if 'viewCount' in st else None
...
def _compute_window_iso(time_filter, custom_from_iso, custom_to_iso):
    if time_filter == "custom":
        ...
        after_dt = {'day': now - dt.timedelta(days=1),
                    'week': now - dt.timedelta(weeks=1),
                    'month': now - dt.timedelta(days=30)}.get(time_filter, dt.datetime(1970,1,1,tzinfo=dt.timezone.utc))
...
def search_via_channel_uploads_fallback(api_key, keyword, channel_ids, published_after_iso, published_before_iso, max_results=50, sort_by="views"):
    """반환: (url, title, date_raw, date_fmt, channel_title, views, dur_s, channel_id)"""
    if build is None: return []
    yt = build('youtube', 'v3', developerKey=api_key)
    t_from = _parse_rfc3339(published_after_iso); t_to = _parse_rfc3339(published_before_iso)
    uploads_by_channel = {}
    for cid in channel_ids:
        try:
            ...
            uploads_by_channel[cid] = (items[0]["contentDetails"]["relatedPlaylists"]["uploads"],
                                       items[0]["snippet"].get("title",""))
        except Exception: continue

    cand_ids, meta = [], {}  # vid -> (title, ch_title, pub_s, cid)
    for cid, (uploads_pl, ch_title0) in uploads_by_channel.items():
        page_token = None; keep = True
        while keep and len(cand_ids) < max_results:
            ...
            for it in resp.get("items", []):
                ...
                if pub_t < t_from: keep = False; break
                if pub_t > t_to: continue
                title = it["snippet"]["title"] or ""; desc = it["snippet"].get("description") or ""
                ch_title = it["snippet"].get("channelTitle") or ch_title0 or ""
                if (keyword.lower() in title.lower()) or (keyword.lower() in desc.lower()):
                    cand_ids.append(vid); meta[vid] = (title, ch_title, pub_s, cid)
                if len(cand_ids) >= max_results: break
    ...
    for i in range(0, len(cand_ids), 50):
        ...
        for v in vr.get("items", []):
            vid = v["id"]; title, ch_title, pub_s, ch_id = meta.get(vid, ("","","",""))
            try:
    ...
    if sort_by == "views": results.sort(key=lambda x: x[5], reverse=True)
    else: results.sort(key=lambda x: x[2], reverse=True)
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("YouTube 자막 추출 & 키워드 검색")

        # ==== 바깥 창 크기 조절 허용 + 기본값/최소값 ====
        self.resizable(True, True)     # 창 테두리 드래그로 가로/세로 리사이즈 허용
        self.geometry("1280x980")      # 시작 크기
        self.minsize(900, 700)         # 최소 크기(원하면 조정)
